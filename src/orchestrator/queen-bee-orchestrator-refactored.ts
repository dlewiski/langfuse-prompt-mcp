...
export class QueenBeeOrchestratorRefactored {
  private config: OrchestratorConfig;
  private static instance: QueenBeeOrchestratorRefactored | null = null;
  ...
  static getInstance(config?: Partial<OrchestratorConfig>): QueenBeeOrchestratorRefactored {
    if (!QueenBeeOrchestratorRefactored.instance) {
      QueenBeeOrchestratorRefactored.instance = new QueenBeeOrchestratorRefactored(config);
    }
    return QueenBeeOrchestratorRefactored.instance;
  }
  ...
  getStatus(): any {
    return {
      config: this.config,
      promptHistory: {
        total: this.phase4.getHistory().length,
        highScoring: this.phase4.getHighScoringCount(),
      },
      ready: true,
    };
  }
...
const orchestrator = QueenBeeOrchestratorRefactored.getInstance();
