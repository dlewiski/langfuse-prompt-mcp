...
if (!langfuse) {
  console.warn('[Config] Langfuse client not initialized due to missing configuration');
}
 ...
 * @returns {number}
 */
const parseIntWithValidation = (value, defaultValue, min = 0, max = Infinity) => {
...
export const CONFIG = Object.freeze({
  ...
  MIN_IMPROVEMENT_SCORE: parseIntWithValidation(
    process.env.MIN_IMPROVEMENT_SCORE, 85, 0, 100
  ),
  
  // Iteration limits
  MAX_ITERATIONS: parseIntWithValidation(
    process.env.MAX_ITERATIONS, 5, 1, 10
  ),
  
  // Pattern extraction
  PATTERN_MIN_SCORE: parseIntWithValidation(
    process.env.PATTERN_MIN_SCORE, 80, 0, 100
  ),
  PATTERN_MIN_OCCURRENCES: parseIntWithValidation(
    process.env.PATTERN_MIN_OCCURRENCES, 3, 1, 100
  ),
  ...
  CACHE_TTL: parseIntWithValidation(
    process.env.CACHE_TTL, 3600000, 0, 86400000  // Default 1 hour, max 24 hours
  ),
  REQUEST_TIMEOUT: parseIntWithValidation(
    process.env.REQUEST_TIMEOUT, 30000, 1000, 120000  // Default 30s, max 2 min
  ),
